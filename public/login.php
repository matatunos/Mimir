    ...
    if (empty($username) || empty($password)) {
        ...
        if (!$security->checkIPRateLimit($clientIP, 'failed_login', 5, 15)) {
            ...
            if ($user && !empty($user['password']) && password_verify($password, $user['password'])) {
                ...
                $stmtIns = $db->prepare("INSERT INTO security_events (event_type, severity, ip_address, user_agent, description, details) VALUES ('failed_login','low', ?, ?, ?, ?)");
                ...
                if ($user && $enableUserLock) {
                    $stmtCount = $db->prepare("SELECT COUNT(*) as attempts FROM security_events WHERE event_type='failed_login' AND details LIKE ? AND created_at > DATE_SUB(NOW(), INTERVAL ? MINUTE)");
                    $pattern = '%"' . $username . '"%';
                    $stmtCount->execute([$pattern, $userLockWindow]);
                    ...
                    if ($attempts >= $userLockThreshold) {
                        ...
                        $stmtRate = $db->prepare("INSERT INTO security_events (event_type, severity, ip_address, user_agent, description, details, action_taken) VALUES ('rate_limit','high', ?, ?, ?, ?, 'user locked')");
                        $rateDetails = json_encode(['username' => $username, 'attempts' => $attempts, 'threshold' => $userLockThreshold]);
                        $stmtRate->execute([$clientIP, $_SERVER['HTTP_USER_AGENT'] ?? 'unknown', 'Usuario bloqueado por intentos fallidos', $rateDetails]);
                    }
                }
